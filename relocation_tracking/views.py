...
#         print("relocation tracker", relocation_df_total.shape[0])
#         print(relocation_df_total.head(100),"-------------------------------")
...
#         # ================= SPLIT DATA =================
#         relocation_df_new_sites = relocation_df_total.loc[
#             relocation_df_total[('AGEING','')].isna()
#         ].copy()

#         relocation_df_old_sites = relocation_df_total.loc[
#             relocation_df_total[('AGEING','')].notna()
#         ].copy()

#         print("relocation_df_new_sites: ", relocation_df_new_sites.shape[0])
#         print("relocation_df_old_sites: ", relocation_df_old_sites.shape[0])
...
#             # ===== AOP =====
#             relocation_df_new_sites.loc[:, ('AOP','')] = relocation_df_new_sites[('AOP','')].fillna('New AOP')

#             print("relocation_df_new_sites: ", relocation_df_new_sites.shape[0])

#             # ===== DATE FIX (CRITICAL) =====
#             col = relocation_df_new_sites[('On Air DATE','')]

#             # ✅ force object dtype (fix for int64 issue)
#             relocation_df_new_sites.loc[:, ('On Air DATE','')] = col.astype('object')
...
#             # normal parsing
#             parsed_dates = pd.to_datetime(col_clean, errors='coerce', dayfirst=True)

#             # handle Excel serial numbers
#             mask = parsed_dates.isna() & col_clean.str.match(r'^\d+(\.\d+)?$')

#             parsed_dates.loc[mask] = pd.to_datetime(
#                 col_clean[mask].astype(float),
#                 unit='D',
#                 origin='1899-12-30'
#             )

#             # assign final
#             relocation_df_new_sites.loc[:, ('On Air DATE','')] = parsed_dates

#             print("Converted Dates----------------------------:")
#             print(relocation_df_new_sites[('On Air DATE','')].head())

#             print("Unique Dates-------------------------------:")
#             print(relocation_df_new_sites[('On Air DATE','')].dropna().unique())

#             # ===== PAYLOAD =====
#             days = ['D-1', 'D-2', 'D-3', 'D-4', 'D-5']
#             print("days........", days)

#             onAirDates = relocation_df_new_sites[('On Air DATE','')].dropna().unique()
#             print("onAirDates: ", onAirDates)
...
#                 print("first_date_data_available: ", first_date_data_available)
#                 print("two_weeks_ago: ", two_weeks_ago)
#                 print("one_week_ago: ", one_week_ago)
#                 print("start_date_two_weeks_ago: ", start_date_two_weeks_ago)
#                 print("start_date_one_week_ago: ", start_date_one_week_ago)
#                 print("onAirDate: ", onAirDate)

#                 if first_date_data_available > start_date_two_weeks_ago:
#                     week_dates_str = [
#                         (start_date_one_week_ago + pd.Timedelta(days=i)).strftime('%d-%b-%y')
#                         for i in range(5)
#                     ]
#                 else:
#                     week_dates_str = [
#                         (start_date_two_weeks_ago + pd.Timedelta(days=i)).strftime('%d-%b-%y')
#                         for i in range(5)
#                     ]
...
#                         relocation_df_new_sites[('On Air DATE','')] == onAirDate, (day, '5G')
#                     ] = relocation_df_new_sites.loc[
#                         relocation_df_new_sites[('On Air DATE','')] == onAirDate, ('Unique_SiteID_Old','')
...
#                     relocation_df_new_sites.loc[
#                         relocation_df_new_sites[('On Air DATE','')] == onAirDate, (day, '4G')
#                     ] = relocation_df_new_sites.loc[
#                         relocation_df_new_sites[('On Air DATE','')] == onAirDate, ('Unique_SiteID_Old','')
...
#                 .replace('-', pd.NA).infer_objects(copy=False)
...
#         print("relocation_df_final: ", relocation_df_final.shape[0])
...
#         relocation_df_final[('On Air DATE','')] = pd.to_datetime(
#                 relocation_df_final[('On Air DATE','')],
#                 errors='coerce'
#             )
...
############################################# SEPERATING OLD SITES AND NEW SITES AND FILLING NEW SITE DATA ##############################################################################
        ...
        if not relocation_df_new_sites.empty:
            ...
            def clean_excel_date(col):
                def parse_date(x):
                    ...
                    # String dates
                    try:
                        return pd.to_datetime(str(x).strip(), dayfirst=True)
                    except:
                        return pd.NaT
            ...
            relocation_df_new_sites[('On Air DATE', '')] = clean_excel_date(
                relocation_df_new_sites[('On Air DATE', '')]
            )
            ...
            for onAirDate in onAirDates:
                ...
                for i, day in enumerate(days):
                    ...
                    relocation_df_new_sites.loc[
                        relocation_df_new_sites[('On Air DATE', '')] == onAirDate,
                        (day, '5G')
                    ] = relocation_df_new_sites.loc[
                        relocation_df_new_sites[('On Air DATE', '')] == onAirDate,
                        ('Unique_SiteID_Old', '')
        ...
        relocation_df_final[('On Air DATE', '')] = clean_excel_date(
            relocation_df_final[('On Air DATE', '')]
        )

        relocation_df_final[("AGEING", "")] = (
            today - relocation_df_final[("On Air DATE", "")]
        ).dt.days
...
############################################################ FORMAT CHANGE FOR DISPLAY #################################################################
        ...
        relocation_df_final[('MS2 Date','')] = clean_excel_date(relocation_df_final[('MS2 Date','')])
