...
def AlarmFileUpload(request):
    ...
    required_columns = [
        "Notification ID", "Alarm Number", "Alarm Type", "Severity", "Alarm Time",
        "Probable Cause", "Probable Cause Code", "Alarm Text", "Distinguished Name",
        "Object Class", "Acknowledgement State", "Acknowledgement Time/UnAcknowledgement Time",
        "Acknowledgement User", "Cancel State", "Cancel Time", "Cancel User",
        "Name", "Maintenance Region Name", "Site Name", "Site Priority", "Site Address",
        "Extra Information", "Diagnostic Info", "User Additional Information",
        "Supplementary Information", "Additional Information 1", "Additional Information 2",
        "Additional Information 3", "Correlation Indicator", "Notes Indicator",
        "Trouble Ticket Indicator", "Alarm Sound", "Instance Counter", "Consecutive Number",
        "Alarm Insertion Time", "Alarm Update Time", "Controlling Object Name",
        "Origin Alarm Time", "Origin Alarm Update Time",
        "Origin Acknowledgement/UnAcknowledgement Time", "Origin Cancel Time", "Related DN's"
    ]
        ...
        for _, row in df.iterrows():
            ...
            if pd.notnull(row["Notification ID"]):
                obj = AlarmNotification(
                    ...
                    AlarmTime=pd.to_datetime(row["Alarm Time"], errors="coerce").tz_localize('utc'),
                    ...
                    AcknowledgementTimeOrUnacknowledgementTime=row["Acknowledgement Time/UnAcknowledgement Time"],
                    ...
                    OriginAcknowledgementOrUnacknowledgementTime=row["Origin Acknowledgement/UnAcknowledgement Time"],
...
def Daily_RAW_KPI_4G(request):
    ...
    try:
        ...
        df['MV eCell Data BH'] = pd.to_datetime(df['MV eCell Data BH'], errors="coerce", format="%H:%M").dt.time
        ...
        return Response({"status": False, "error": f"Data preparation error: {str(e)}"}, status=500)

    present_dates = df["Date"].unique()
    present_dates = pd.to_datetime(present_dates).strftime("%Y-%m-%d").tolist()
    exists = Daily_4G_KPI.objects.filter(Date__in=present_dates).values_list("Date", flat=True)
    kpiStrToZero = ['MV_UL User Throughput_Kbps [CDBH]','MV_DL User Throughput_Kbps [CDBH]']
            ...
            for _, row in df.iterrows():
                ...
                if pd.notnull(row["Date"]):
                    obj = Daily_4G_KPI(
                        ...
                        MV_Average_number_of_used_DL_PRBs=row["MV_Average number of used DL PRBs [CDBH]"],
                        ...
                        MV_PS_handover_success_rate_LTE_INTER_SYSTEM=row["MV_PS handover success rate [LTE Inter System] [CDBH]"],
                        MV_PS_handover_success_rate_LTE_INTRA_SYSTEM=row["MV_PS handover success rate [LTE Intra System] [CDBH]"],
...
def tantitive_counters_save_data(request):
    ...
    try:
        df = pd.read_csv(file)
        df["Short name"] = df["Short name"].fillna(method="ffill")
        df['Nokia_S1_Failures_MME_Issue'] = df['Nokia_S1_Failures_MME_Issue'].apply(lambda x: np.nan if isinstance(x, str) else x)
        df['S1_Failures_MME_Issue'] = df['S1_Failures_MME_Issue'].apply(lambda x: np.nan if isinstance(x, str) else x)
    ...
    required_columns = [
        "Short name","DateTime",
        "S1 Failure CDBH",
        "Nokia_S1_Failures_MME_Issue", "S1_Failures_MME_Issue", "MME Generated Issues on Cells",
        "LS1AP.S1AP_PARTIAL_RESET_INIT_MME", "LS1AP.S1AP_KILL_REQ", "LS1AP.S1AP_KILL_RESP",
        "LRRC.REJ_RRC_CONN_RE_ESTAB", "LRRC.RRC_PAGING_REQUESTS", "LRRC.RRC_CON_RE_ESTAB_ATT_HO_FAIL",
        "LRRC.RRC_CON_RE_ESTAB_ATT_OTHER", "RRC_CONN_REL_TA_LIMIT", "LEPSB.EPC_EPS_BEARER_REL_REQ_NORM",
        "LEPSB.EPC_EPS_BEARER_REL_REQ_DETACH", "LEPSB.EPC_EPS_BEARER_REL_REQ_RNL", "LEPSB.EPC_EPS_BEARER_REL_REQ_OTH",
        
        "LEPSB.ERAB_NBR_DL_FAIL_OVL", "LEPSB.ERAB_NBR_UL_FAIL_OVL",
      "LEPSB.ERAB_REL_DOUBLE_S1",
        "LEPSB.ERAB_REL_ALL_ABNORMAL_H_PWR_UE", "LXPL.SCG_ADD_COMPLETION_FAIL_T",
        "LXPL.SGNB_ADD_COMPL_FAIL_T", "LXPL.SCG_MOD_ADD_MENB_COMPL_FAIL_T", "NX2CC.NX2CC_RAB_REL_ATT_MENB",
        "NX2CC.NX2CC_RAB_REL_NORM_MENB", "NX2CC.NX2CC_RAB_REL_ATT_SGNB", "NX2CC.NX2CC_RAB_REL_ABNORM_SGNB",
        "NX2CC.NX2CC_RAB_REL_PREEM_SGNB", "RSRP Samples<-116 dBm", "TNL Failure Count", "TNL Failure %",
        "LRRC.DISC_RRC_PAGING", "MV_Radio NW Availability", "UL RSSI","Bler UL", "Bler DL", "LEPSB.ERAB_REL_ENB_TNL_TRU","LEPSB.ERAB_INI_SETUP_FAIL_TNL_TRU",
        'LEPSB.ERAB_INI_SETUP_FAIL_RNL_UEL',"LEPSB.ERAB_INI_SETUP_FAIL_RNL_RRNA","LEPSB.ERAB_INI_SETUP_FAIL_RNL_RIP","LUEST.SIGN_EST_F_RRCCOMPL_MISSING",
        "LUEST.SIGN_CONN_ESTAB_FAIL_PUCCH","LUEST.SIGN_CONN_ESTAB_FAIL_MAXRRC",
    ...
    for _, row in df.iterrows():
        try:
            ...
            obj = Tantitive_Counters_24_Hours(
                ...
                LS1AP_S1AP_PARTIAL_RESET_INIT_MME=row["LS1AP.S1AP_PARTIAL_RESET_INIT_MME"],
                ...
                LRRC_RRC_CON_RE_ESTAB_ATT_HO_FAIL=row["LRRC.RRC_CON_RE_ESTAB_ATT_HO_FAIL"],
                LRRC_RRC_CON_RE_ESTAB_ATT_OTHER=row["LRRC.RRC_CON_RE_ESTAB_ATT_OTHER"],
                RRC_CONN_REL_TA_LIMIT=row["RRC_CONN_REL_TA_LIMIT"],
                LEPSB_EPC_EPS_BEARER_REL_REQ_NORM=row["LEPSB.EPC_EPS_BEARER_REL_REQ_NORM"],
                LEPSB_EPC_EPS_BEARER_REL_REQ_DETACH=row["LEPSB.EPC_EPS_BEARER_REL_REQ_DETACH"],
                LEPSB_EPC_EPS_BEARER_REL_REQ_RNL=row["LEPSB.EPC_EPS_BEARER_REL_REQ_RNL"],
                LEPSB_EPC_EPS_BEARER_REL_REQ_OTH=row["LEPSB.EPC_EPS_BEARER_REL_REQ_OTH"],
                ...
                LEPSB_ERAB_REL_ALL_ABNORMAL_H_PWR_UE=row["LEPSB.ERAB_REL_ALL_ABNORMAL_H_PWR_UE"],
                ...
                LXPL_SCG_MOD_ADD_MENB_COMPL_FAIL_T=row["LXPL.SCG_MOD_ADD_MENB_COMPL_FAIL_T"],
                ...
                UL_BLER = row['Bler UL'],
                DL_BLER = row['Bler DL'],
                LUEST_SIGN_CONN_ESTAB_FAIL_MAXRRC=row['LUEST.SIGN_CONN_ESTAB_FAIL_MAXRRC'],
                LUEST_SIGN_CONN_ESTAB_FAIL_PUCCH=row['LUEST.SIGN_CONN_ESTAB_FAIL_PUCCH'],
                LUEST_SIGN_EST_F_RRCCOMPL_MISSING=row['LUEST.SIGN_EST_F_RRCCOMPL_MISSING'],
                LEPSB_ERAB_INI_SETUP_FAIL_RNL_RIP=row['LEPSB.ERAB_INI_SETUP_FAIL_RNL_RIP'],
                LEPSB_ERAB_INI_SETUP_FAIL_RNL_RNA=row['LEPSB.ERAB_INI_SETUP_FAIL_RNL_RRNA'],
                LEPSB_ERAB_INI_SETUP_FAIL_RNL_UEL=row['LEPSB.ERAB_INI_SETUP_FAIL_RNL_UEL'],
                LEPSB_ERAB_INI_SETUP_FAIL_TNL_TRU=row['LEPSB.ERAB_INI_SETUP_FAIL_TNL_TRU'],
...
def save_df_to_model(df,date):
    ...
    expected_columns = [
        'Cell_name', 'OEM_GGSN' ,'KPI', 'cell_value', 'threshold_value', 'check',
        'RCA', 'Proposed Solution', 'history_alarms', 'circle'
    ]
    ...
    existing_objs = RCA_output_table.objects.filter(Cell_name__in=[x[0] for x in unique_values], date=date)
    ...
    if objs_to_update:
        RCA_output_table.objects.bulk_update(objs_to_update, [
            'cell_value',
            'threshold_value',
            'check_condition',
            'RCA',
...
def accessibility_rca(KPI_4G_record_df,KPI_row,alarm_data_df,tentative_counter_df):
    def perticular_condotion_check(KPI,cell_record,alarm_data_df, tentative_counter_df):
        ...
        if mv_cell_data_BH is None:
            return ["MV eCell Data BH is None in daily 4G KPI table"],solution,history_alarms

        tantative_cell_record = tentative_counter_df[(tentative_counter_df['Short_name'] == short_name) & (tentative_counter_df['DateTime'].dt.time  == mv_cell_data_BH)]
        ...
        if tantative_cell_record.empty:
            return [f" Cell not found in tentative counter table"],solution,history_alarms
        ...
        for i,row in RCA_TABLE_df.iterrows():
            ...
            if row['Data_source'] == 'MYCOM':
                ...
                if OPERATORS_dict[row.Operator](tantative_cell_record_series[row['Tentative_counters']],row["Condition_check"]):
                ...
                combine_datetime = datetime.combine(cell_record['Date'], cell_record['MV_eCell_Data_BH'])

                # print(combine_datetime)
                
                start_datetime,end_datetime = calculate_datetime_range(combine_datetime)
                alarm_data_df_active = alarm_data_df[(alarm_data_df["MRBTS_ID"] == mrbts_id) & (alarm_data_df['AlarmTime'].dt.tz_localize(None) <= end_datetime) & alarm_data_df['CancelState'] == 'False']
                ...
                # exit(0)
                supplementry_information = alarm_data_df_active['SupplementaryInformation'].to_list()
                ...
                ############### for history alarms ##################
                alarm_data_df_history = alarm_data_df[(alarm_data_df["MRBTS_ID"] == mrbts_id) & (alarm_data_df['CancelState'] == 'True') & (alarm_data_df['AlarmTime'].dt.date == cell_record['Date'])]
                # alarm_data_df.to_csv("alarm_aaram_data.csv"   T``      
                # \G             # exit(0)
                ...
                # exit(0)
                supplementry_information = alarm_data_df_history['SupplementaryInformation'].to_list()
        ...
        #  print(cell_record['Short_name'])
                ...
                #  print(KPI_row['KPI'])
                 ...
                 if  OPERATORS_dict[KPI_row.operator](cell_record[kpi],KPI_row.threshold_value):
                    ...
                    #  print("true")
                     rca_report.append({'Cell_name':cell_record['Short_name'],"OEM_GGSN":cell_record['OEM_GGSN'],'KPI':kpi, "cell_value": cell_record[kpi], "threshold_value": KPI_row.threshold_value,'check':'OK','RCA':'',"Proposed Solution":'',"history_alarms":''})
                 else:
                     rca,solution,history_alarms = perticular_condotion_check(kpi,cell_record,alarm_data_df, tentative_counter_df)
                    ...
                    #  print(rca_str,solution_str)
                     rca_report.append({'Cell_name':cell_record['Short_name'],"OEM_GGSN":cell_record['OEM_GGSN'], 'KPI':kpi, "cell_value": cell_record[kpi], "threshold_value": KPI_row.threshold_value,'check':'NOT OK','RCA':rca_str,"Proposed Solution":solution_str,"history_alarms":history_str})
                    #  print(KPI_row.operator)
                    #  print("cell_value; ",cell_record[kpi]," threshold_value: ",KPI_row.threshold_value)
                    #  print('false')
    return rca_report                 
...
def payload_rca(latest_date,alarm_data_df):
    ...
    def check_condition(cell_record,alarm_data_df):
        ...
        for i,row in RCA_payload_table_df.iterrows():
            if row['Data_source'] == 'MYCOM':
                if row['Data_source'] == 'MYCOM':
                    ...
                    if not (row.Operator == "=<=<"):
                        if OPERATORS_dict[row.Operator](cell_record[row['Tentative_counters']],row["Condition_check"]):
                        # print("______condition_check__")
                        
                            rca.append(row["RCA"])
                            solution.append(row['Proposed_solution'])
                    else:
                        if OPERATORS_dict[row.Operator](cell_record[row['Tentative_counters']],-30,30):
                        # print("______condition_check__")
                ...
                mrbts_id = cell_record['MRBTS_ID']
                # print(cell_record['Short_name'],mrbts_id)
                alarm_data_df_active = alarm_data_df[(alarm_data_df["MRBTS_ID"] == mrbts_id) & alarm_data_df['CancelState'] == 'False']
                # alarm_data_df.to_csv("alarm_aaram_data.csv"   T``      
                # \G             # exit(0)
                if alarm_data_df_active.empty:
                    print("empty alarm file")
                ...
                ############### for history alarms ##################
                alarm_data_df_history = alarm_data_df[(alarm_data_df["MRBTS_ID"] == mrbts_id) & (alarm_data_df['CancelState'] == 'True') & (alarm_data_df['AlarmTime'].dt.date == cell_record['Date_N'])]
                # alarm_data_df.to_csv("alarm_aaram_data.csv"   T``      
                # \G             # exit(0)
                if alarm_data_df_history.empty:
                    print("empty alarm file")
        ...
        return rca,solution,history_alarms
    rca_report=[]
    response = requests.get("http://103.242.225.195:8000/Zero_Count_Rna_Payload_Tool/Date_Wise_Dashboard/")
    data=response.json()
    date_n=latest_date
    ...
    if df_N.empty or df_N_1.empty:
        ...
        merged_df=pd.merge(df_N,df_N_1,on='Short_name',how='left',suffixes=['_N','_N_1'])
        # for cell in merged_df.columns:
        #     print(cell)
        merged_df['dev ue distance'] = round((((merged_df['MV_Average_UE_Distance_KM_N'] - merged_df['MV_Average_UE_Distance_KM_N_1']))/merged_df['MV_Average_UE_Distance_KM_N_1'])*100,2)
        merged_df['dev RS Power [dB]'] = merged_df['RS_Power_dB_N'] - merged_df['RS_Power_dB_N_1']
        merged_df['dev dlRsBoost'] = merged_df['dlRsBoost_N'] - merged_df['dlRsBoost_N_1']

        merged_df['dev UL RSSI'] = np.where((merged_df['UL_RSSI_N'] - merged_df['UL_RSSI_N_1'])<0,
                                    0,
                                    np.where(merged_df['UL_RSSI_N']>-102,merged_df['UL_RSSI_N'] - merged_df['UL_RSSI_N_1'],0))
        op_df=merged_df[['Short_name','Date_N','ECGI_4G_N','MV_Radio_NW_Availability_N','dev UL RSSI','dev dlRsBoost','dev RS Power [dB]','dev ue distance']].fillna(value=0)
        ...
        for i,cell_record in op_df.iterrows():
            if cell_record['Short_name'] not in  pyl_dip_cell:
                rca_report.append({'Cell_name':cell_record['Short_name'],'KPI':kpi, "cell_value": 0, "threshold_value": 0,'check':'OK','RCA':'',"Proposed Solution":'',"history_alarms":''})
            else:
                     rca,solution,history_alarms = check_condition(cell_record,alarm_data_df)
                    ...
                    #  print(rca_str,solution_str)
                     rca_report.append({'Cell_name':cell_record['Short_name'],'KPI':kpi, "cell_value": 0, "threshold_value":0 ,'check':'NOT OK','RCA':rca_str,"Proposed Solution":solution_str,"history_alarms":history_str})
                    #  print(KPI_row.operator)
                    #  print("cell_value; ",cell_record[kpi]," threshold_value: ",KPI_row.threshold_value)
                    #  print('false')
    return rca_report
...
def throupt_rca(KPI_4G_record_df, collable_date,tentative_counter_df, KPI_row):
    ...
    def process_tentative_counter_df(api_url, data, tentative_counter_df):
        ...
        tentative_counter_df['DateTime'] = pd.to_datetime(tentative_counter_df['DateTime'])
    ...
    def process_MV_E_UTRAN_Average_CQI(api_url, data):
        ...
        cqi_day_data_df.drop(["date_1_val","date_2_val","date_3_val"], inplace=True,axis = 1) 
        cqi_day_data_df = cqi_day_data_df.drop_duplicates(subset='Short_name', keep='first')
        columns_to_rename = ['week_1_val', 'week_2_val', 'date_4_val', 
                            'date_5_val', 'date_6_val', 'date_7_val', 'date_8_val']

        # Renaming with a suffix
        cqi_day_data_df = cqi_day_data_df.rename(columns={col: f"{col}_CQI" for col in columns_to_rename})
    ...
    def process_MV_Radio_NW_Availability(api_url, data):
        ...
        rna_5day_data_df = rna_5day_data_df.drop_duplicates(subset='Short_name', keep='first')

        rna_5day_data_df.drop(["date_1_val","date_2_val","date_3_val"], inplace=True,axis = 1) 
        ...
        # Renaming with a suffix
        rna_5day_data_df = rna_5day_data_df.rename(columns={col: f"{col}_rna" for col in columns_to_rename})
    ...
    def process_ul_rssi(api_url, data):
        ...
        if response.status_code == 200:
            ...
            print(f"Error: {response.status_code}")
        ul_rssi_day_data_df = ul_rssi_day_data_df.drop_duplicates(subset='Short_name', keep='first')
        ul_rssi_day_data_df.drop(["date_1_val","date_2_val","date_3_val"], inplace=True,axis = 1) 
        ...
        # Renaming with a suffix
        ul_rssi_day_data_df = ul_rssi_day_data_df.rename(columns={col: f"{col}_rssi" for col in columns_to_rename})
    ...
    def process_MV_Average_number_of_used_DL_PRBs(api_url, data):
        ...
        if response.status_code == 200:
            ...
            print(f"Error: {response.status_code}")
        prb_day_data_df
        prb_day_data_df = prb_day_data_df.drop_duplicates(subset='Short_name', keep='first')
        prb_day_data_df.drop(["date_1_val","date_2_val","date_3_val"], inplace=True,axis = 1) 
        prb_day_data_df['max_prb_among _5_day'] = prb_day_data_df[['date_4_val', 'date_5_val', 'date_6_val', 'date_7_val', 'date_8_val']].max(axis=1)
        columns_to_rename = ['week_1_val', 'week_2_val', 'date_4_val', 
                            'date_5_val', 'date_6_val', 'date_7_val', 'date_8_val']

        # Renaming with a suffix
        prb_day_data_df = prb_day_data_df.rename(columns={col: f"{col}_prb" for col in columns_to_rename})
    ...
    def process_throupt_data(api_url, data):
        ...
        thpt_5day_data_df.drop(["week_1_val","week_2_val","date_1_val","date_2_val","date_3_val"], inplace=True,axis = 1) 
        thpt_5day_data_df[['date_4_val', 'date_5_val', 'date_6_val', 'date_7_val', 'date_8_val']] /= 1000
        thpt_5day_data_df['more_than_3_values'] = (
            (thpt_5day_data_df[['date_4_val', 'date_5_val', 'date_6_val', 'date_7_val', 'date_8_val']] >= 3).sum(axis=1) >= 3 )
        thpt_5day_data_df["worst_thpt"] = thpt_5day_data_df[['date_4_val', 'date_5_val', 'date_6_val', 'date_7_val', 'date_8_val']].idxmin(axis=1)


        thpt_5day_data_df = thpt_5day_data_df.drop_duplicates(subset='Short_name', keep='first')
        thpt_5day_data_df["sector"] = thpt_5day_data_df['Short_name'].str.split('_').str[-2].str[-1]

        columns_to_rename = ['week_1_val', 'week_2_val', 'date_4_val', 
                            'date_5_val', 'date_6_val', 'date_7_val', 'date_8_val']

        # Renaming with a suffix
        thpt_5day_data_df = thpt_5day_data_df.rename(columns={col: f"{col}_thpt" for col in columns_to_rename})
    ...
    removed_nokia_NE_ik_nospace_df=removed_nokia_NE_ik_df[removed_nokia_df['Short_name'].str.contains('_') | removed_nokia_df['Short_name'].str.startswith(',')]
    condition_sams= removed_nokia_NE_ik_nospace_df['Short_name'].str.startswith('Sams')
    removed_nokia_NE_ik_nospace_sams_df=removed_nokia_NE_ik_nospace_df[~condition_sams]
    removed_nokia_NE_ik_nospace_sams_df['site_id'] = removed_nokia_NE_ik_nospace_sams_df['Short_name'].apply(lambda x: x.split('_')[-3][:-1] if '_FDD' in x or '_TDD' in x else x.split('_')[-2][:-1])
    removed_nokia_NE_ik_nospace_sams_df['sector'] = removed_nokia_NE_ik_nospace_sams_df['Short_name'].str.split('_').str[-2].str[-1]
    removed_nokia_NE_ik_nospace_sams_df = removed_nokia_NE_ik_nospace_sams_df.sort_values(by=['site_id', 'sector', 'MV_Freq_Band'])
    columns_order = ['site_id', 'sector','MV_Freq_Band'] + [col for col in removed_nokia_NE_ik_nospace_sams_df.columns if col not in ['site_id', 'sector','MV_Freq_Band']]
    removed_nokia_NE_ik_nospace_sams_df = removed_nokia_NE_ik_nospace_sams_df[columns_order]
    removed_nokia_NE_ik_nospace_sams_df['MV_DL_User_Throughput_mbps'] = removed_nokia_NE_ik_nospace_sams_df['MV_DL_User_Throughput_Kbps'] / 1000
    removed_nokia_NE_ik_nospace_sams_df['MV_Freq_Band'] =removed_nokia_NE_ik_nospace_sams_df['MV_Freq_Band'].astype(float)
    ...
    cell_t_f_dict=thpt_5day_data_df[["Short_name","more_than_3_values"]].set_index("Short_name")["more_than_3_values"].to_dict()
    
    
    # To find the ok and not ok value based on the 5 day data of the thpt
    # Create an empty DataFrame to store the results

    results_df = pd.DataFrame(columns=['site_id', 
                                    'sector',
                                    'cell_name',
                                    'MV_Freq_Band',
                                    'MV_Freq_Bandwidth',
    ...
    for site_id in removed_nokia_NE_ik_nospace_sams_df['site_id'].unique():
        site_data = removed_nokia_NE_ik_nospace_sams_df[removed_nokia_NE_ik_nospace_sams_df['site_id'] == site_id]
        ...
        for sector in site_data['sector'].unique():
            ...
            for freq_band in sector_data['MV_Freq_Band'].unique():
                ...
                for index, row in freq_band_data.iterrows():
                    if cell_t_f_dict.get(row['Short_name']) or  (row['MV_Freq_Band'] == 900 or row['MV_Freq_Band'] == 850):
                        remark = 'ok'
                    else:
                        remark = 'not ok'
                    ...
                    results_df = pd.concat([results_df, pd.DataFrame([{
                        ...
                        'MV_Radio_NW_Availability':row['MV_Radio_NW_Availability'],
    ...
    thpt_5data_for_merge = thpt_5day_data_df[['Short_name','date_4_val_thpt', 'date_5_val_thpt', 'date_6_val_thpt', 'date_7_val_thpt', 'date_8_val_thpt',"worst_thpt"]]
    results_df = pd.merge(results_df, thpt_5data_for_merge, left_on='cell_name', right_on='Short_name', how='left')
    results_df = results_df[['site_id', 'sector', 'cell_name', 'MV_Freq_Band','MV_eCell_Data_BH', 'MV_Freq_Bandwidth','MV_Radio_NW_Availability','date_4_val_thpt', 'date_5_val_thpt', 'date_6_val_thpt','date_7_val_thpt', 'date_8_val_thpt',"worst_thpt",'remark']]


    results_df['MV_Freq_Bandwidth'] =results_df['MV_Freq_Bandwidth'].str.rstrip("MHz").str.strip()
    ...
    results_df = pd.merge(results_df, prb_day_data_df[["Short_name","max_prb_among _5_day",'week_1_val_prb', 'week_2_val_prb', 'date_4_val_prb', 'date_5_val_prb','date_6_val_prb', 'date_7_val_prb', 'date_8_val_prb']], left_on='cell_name', right_on='Short_name', how='left')
    ...
    results_df = pd.merge(results_df, ul_rssi_day_data_df[["Short_name",'week_1_val_rssi', 'week_2_val_rssi', 'date_4_val_rssi',
       'date_5_val_rssi', 'date_6_val_rssi', 'date_7_val_rssi',
       'date_8_val_rssi']], left_on='cell_name', right_on='Short_name', how='left')
    ...
    results_df = pd.merge(results_df, rna_5day_data_df[["Short_name",'week_1_val_rna', 'week_2_val_rna', 'date_4_val_rna',
       'date_5_val_rna', 'date_6_val_rna', 'date_7_val_rna',
       'date_8_val_rna']], left_on='cell_name', right_on='Short_name', how='left')
    
    results_df['rna_on_worst_thpt_date'] =results_df.apply(lambda row: row[row['worst_thpt'] + '_rna'], axis=1 )
    ...
    results_df = pd.merge(results_df, cqi_day_data_df, left_on='cell_name', right_on='Short_name', how='left')

    results_df["worst_thpt"] = results_df[['date_4_val_thpt', 'date_5_val_thpt', 'date_6_val_thpt', 'date_7_val_thpt', 'date_8_val_thpt']].idxmin(axis=1)
    ...
    results_df["worst_thpt_original_date"] = results_df["worst_thpt"].apply(lambda x:column_mapping[x])
    results_df["worst_thpt_original_date"] = pd.to_datetime(results_df["worst_thpt_original_date"])
    results_df["worst_thpt_original_date"] = results_df["worst_thpt_original_date"].dt.date
    ...
    data={"dates":date_columns,"columns":["LEPSB_ERAB_INI_SETUP_FAIL_RNL_RIP","TNL_Failure_Percent","RSRP_Samples_lt_116_dBm"]}
    
    tentative_counter_df = process_tentative_counter_df(api_url, data, tentative_counter_df)
    results_df.drop("Short_name", inplace= True, axis = 1)
    results_df = results_df.merge(tentative_counter_df, left_on=["cell_name","worst_thpt_original_date","MV_eCell_Data_BH"],right_on=["Short_name","Date","Time"], how="left")
    ...
    for index, row in results_df.iterrows():
        if row['remark'] == 'not ok':
            ...
            throughput_check = results_df[(results_df['site_id'] == site) & 
            ...
            if not throughput_check.empty:
                results_df.loc[index, ['RCA','Solution']] = ['>5 mbps on other layer within the same sector','Load Balancing required']
            worst_thpt_cqi = worst_thpt.rstrip("_thpt") + "_CQI" 
            # print(worst_thpt_cqi)
            if row[worst_thpt_cqi] < 7.5:
                results_df.loc[index, 'RCA']= str(results_df.loc[index, 'RCA']) + ',CQI Poor'
                results_df.loc[index, 'Solution'] = str(results_df.loc[index, 'Solution']) + ',Optimization for Cqi improvement'
            ...
            # logic for prb....        
            prb_day_data_series = prb_day_data_df[prb_day_data_df["Short_name"] == row["cell_name"]].iloc[0]
            
            rna_check_on_not_ok_cell_on_wrst_thpr_df = results_df[(results_df['site_id'] == site) & 
                                        (results_df['sector'] == sector) & 
                                        (results_df['cell_name'] != row['cell_name']) &
                                        (~results_df['MV_Freq_Band'].isin([900, 850]))][[worst_thpt.rstrip("_thpt") + "_rna"]]  
            rna_check_on_not_ok_cell_on_wrst_thpr_df =rna_check_on_not_ok_cell_on_wrst_thpr_df[rna_check_on_not_ok_cell_on_wrst_thpr_df[worst_thpt.rstrip("_thpt") + "_rna"] < 80]
            
            if prb_day_data_series["max_prb_among _5_day"] > 75 :
                    results_df.loc[index, 'RCA']= str(results_df.loc[index, 'RCA']) + ', High PRB Utilization'
                    results_df.loc[index, 'Solution'] = str(results_df.loc[index, 'Solution']) + ', need to set solution.'
                
            if prb_day_data_series['week_1_val_prb'] > 2 :
            
                percentage_change =(prb_day_data_series["max_prb_among _5_day"] - prb_day_data_series["week_1_val_prb"]) 
                if prb_day_data_series["Short_name"] == "MU_E_F3_OM_1990A_A":
                    print("max_val: ",prb_day_data_series["max_prb_among _5_day"], "pre week val: ",prb_day_data_series["week_1_val_prb"])
                    print(percentage_change)
                if prb_day_data_series["max_prb_among _5_day"] > 75  and percentage_change > 5 :
                    if not rna_check_on_not_ok_cell_on_wrst_thpr_df.empty :
                        results_df.loc[index, 'RCA']= str(results_df.loc[index, 'RCA']) + ', payload Prb increased on cell due to RNA issue'
                        results_df.loc[index, 'Solution'] = str(results_df.loc[index, 'Solution']) + ', RNA correction with in sector.'
                        
                    else:
                        results_df.loc[index, 'RCA']= str(results_df.loc[index, 'RCA']) + ', payload Prb increased'
                        results_df.loc[index, 'Solution'] = str(results_df.loc[index, 'Solution']) + ', need to set solution.'
                        
                    
            # logic for rssi.....
            
            worst_thpt_rssi = thpt_5day_data_df[thpt_5day_data_df["Short_name"] == row["cell_name"]]["worst_thpt"].unique()
            worst_thpt_rssi = worst_thpt_rssi[0] + "_rssi"
            # print("rssi:",worst_thpt)
            ul_rssi = ul_rssi_day_data_df[ul_rssi_day_data_df["Short_name"] == row["cell_name"]].iloc[0][worst_thpt_rssi]
            # print(ul_rssi)
            if ul_rssi >= -100:
                results_df.loc[index, 'RCA']= str(results_df.loc[index, 'RCA']) + ', High Rssi'
                results_df.loc[index, 'Solution'] = str(results_df.loc[index, 'Solution']) + ', HW troubleshooting or Interfernce improvement.'
            # logic for LEPSB.ERAB_INI_SETUP_FAIL_RNL_RIP.....
            if row["LEPSB_ERAB_INI_SETUP_FAIL_RNL_RIP"] > 50:
                results_df.loc[index, 'RCA']= str(results_df.loc[index, 'RCA']) + ', RNL fail high'
                results_df.loc[index, 'Solution'] = str(results_df.loc[index, 'Solution']) + ', Coverage/ cell overlapping  improvement.'
            if row["TNL_Failure_Percent"] > 5:
                results_df.loc[index, 'RCA']= str(results_df.loc[index, 'RCA']) + ', TNL fails high'
                results_df.loc[index, 'Solution'] = str(results_df.loc[index, 'Solution']) + ', need to set solution.'
            if row["RSRP_Samples_lt_116_dBm"] > 15:
                results_df.loc[index, 'RCA']= str(results_df.loc[index, 'RCA']) + ', Sample coming from poor coverage'
                results_df.loc[index, 'Solution'] = str(results_df.loc[index, 'Solution']) + ', need to set solution.'

            # logic for >5 mb not available on other layers with in the same sector and PRB >70 for all the layers within the sector.....
            no_gt_5mb_thpt_on_other_layers_check = results_df[(results_df['site_id'] == site) & 
                                (results_df['sector'] == sector) & 
                                (results_df['cell_name'] != row['cell_name']) & 
                                (results_df[worst_thpt] > 5) & 
                                (~results_df['MV_Freq_Band'].isin([900, 850]))]
            prb_gt_70_on_other_layers_on_wrst_thpt_check = results_df[(results_df['site_id'] == site) & 
                                (results_df['sector'] == sector) &  
                                (~results_df['MV_Freq_Band'].isin([900, 850]))]

            if no_gt_5mb_thpt_on_other_layers_check.empty and (prb_gt_70_on_other_layers_on_wrst_thpt_check[worst_thpt.rstrip("_thpt") + "_prb"] > 70).all():
                results_df.loc[index, 'RCA']= str(results_df.loc[index, 'RCA']) + ', High utilization'
                results_df.loc[index, 'Solution'] = str(results_df.loc[index, 'Solution']) + ', Capacity addition required.'
    ...
    for i, cell_record in results_df.iterrows():
        ...
        if  cell_record['remark'] == 'ok':
            ...
            #  print("cell_value; ",cell_record[kpi]," threshold_value: ",KPI_row.threshold_value)
            #  print("true")
             rca_report.append({'Cell_name':cell_record['Short_name'],"OEM_GGSN":cell_record['OEM_GGSN'],'KPI':kpi, "cell_value": '', "threshold_value": KPI_row.threshold_value,'check':'OK','RCA':'',"Proposed Solution":'',"history_alarms":''})
        else:
            rca_report.append({'Cell_name':cell_record['Short_name'],"OEM_GGSN":cell_record['OEM_GGSN'], 'KPI':kpi, "cell_value": '', "threshold_value": KPI_row.threshold_value,'check':'NOT OK','RCA':cell_record['RCA'],"Proposed Solution":cell_record['Solution'],"history_alarms":''})
...
def process_remove_duplicates(data):
    ...
    data = data[(data["mark_value"] != "partial") & (data["mark_value"] != "complete_empty")]
...
def main_process(request):
    ...
    filtered_records_df = Daily_4G_KPI.objects.filter(Date = latest_record_Date_df).values().order_by('Short_name')
    ...
    tentative_counter = Tantitive_Counters_24_Hours.objects.filter(DateTime__date = latest_record_Date_df ).values()
    ...
    alarm_data= AlarmNotification.objects.filter(Upload_date = latest_record_Date_df).values()
    ...
    # Iterate over the filtered recordsl
        ...
        if KPI_row['KPI'] in ['MV_RRC_Setup_Success_Rate','MV_ERAB_Setup_Success_Rate']:
             print("Working for kpi:- 'Accessibility' Analysis...")
            #  print('Working for accessibility parameters...')
             access_rca=accessibility_rca(KPI_4G_record_df,KPI_row,alarm_data_df,tentative_counter_df)
            ...
            dl_throupt = throupt_rca(KPI_4G_record_df, latest_record_Date_df,tentative_counter_df, KPI_row)
    ...
    removed_nokia_NE_ik_df = removed_nokia_df[~condition_NE_ik]
    removed_nokia_NE_ik_nospace_df=removed_nokia_NE_ik_df[removed_nokia_df['Cell_name'].str.contains('_') | removed_nokia_df['Cell_name'].str.startswith(',')]
    ...
    def extract_circle(cell):
        ...
        return circle
    removed_nokia_NE_ik_nospace_df['circle']=removed_nokia_NE_ik_nospace_df["Cell_name"].apply(extract_circle)
 
    # Cell 3

    removed_nokia_NE_ik_nospace_df=removed_nokia_NE_ik_nospace_df[['circle','OEM_GGSN','Cell_name', 'KPI', 'cell_value', 'threshold_value', 'check', 'RCA',
       'Proposed Solution', 'history_alarms']]
    ...
    removed_nokia_NE_ik_nospace_df.to_csv(os.path.join(MEDIA_ROOT, "RCA_TOOL","output" ,f"rca_report_{latest_record_Date_df}.csv"),index=False)
    download_path = os.path.join(MEDIA_URL, "RCA_TOOL", "output",f"rca_report_{latest_record_Date_df}.csv")
...
def latest_record_data(request):
    ...
    latest_tentative_counter_date = Tantitive_Counters_24_Hours.objects.latest("DateTime").DateTime.date()
    latest_alarm_date = AlarmNotification.objects.latest("Upload_date").Upload_date

    return Response({"latest_4G_KPI_date":latest_4G_KPI_date, "latest_tentative_counter":latest_tentative_counter_date, "latest_alarm_date":latest_alarm_date})
...
def get_rca_output_dated(request):
    ...
    if date and cell_names:
        ...
        rca_output = RCA_output_table.objects.filter(date=date, Cell_name__in=cell_names,KPI = 'MV_4G_Data_Volume_GB_N')

        if rca_output.exists():
            serializer = CellDataSerializer(rca_output, many=True)
            return Response({'data':serializer.data}, status=status.HTTP_200_OK)
        else:
            return Response({"message": "No data found for the provided parameters."}, status=status.HTTP_404_NOT_FOUND)
    else:
        return Response({"message": "Both 'date' and 'cell_name' parameters are required."}, status=status.HTTP_400_BAD_REQUEST)
...
def filter_tantitive_data_postgres(request):
    try:
        ...
        # Validate columns
        valid_fields = {field.name for field in Tantitive_Counters_24_Hours._meta.fields}
        ...
        selected_columns = ', '.join(['"DateTime"', '"Short_name"'] + [f'"{col}"' for col in columns])
