...
def _build_at_summary(df, start_dt, end_dt):
    ...
    df_range = _apply_circle_combine(df_range)
    ...
    # ── Step 4: sort by Circle, then SR. No. numerically where possible ───
    def _sort_key(item):
        circle, sr = item[0], item[1]
        try:
...
def performance_at_sr_wise_tracking(request):
    ...
    Body:
    {
        "start_date":    "2025-12-26",
        "end_date":      "2026-01-25",
        "circle":        "UP East",        # optional — filter by circle
        "status_filter": "PAT"             # optional — only rows where PAT == 'Pending'
    }
    """
    month_label    = request.data.get("month", "").strip()
    start_str     = request.data.get("start_date", "").strip()
    ...
    status_filter = request.data.get("status_filter", "").strip()  # PAT / SAT / KAT / SCFT
    site_id_filter = request.data.get("site_id",       "").strip()
    #print(f"DEBUG: site_id_filter = '{site_id_filter}'")
    ...
    if month_label:
        ...
        end_dt       = _bucket_end(month_label)
        safe_label   = month_label.replace(' ', '_')
        out_filename = f"output_AT_Report_{safe_label}.xlsx"
    else:
        start_dt, end_dt, err = _validate_date_range(start_str, end_str)
        if err:
            return err
        out_filename = f"output_AT_Report_{start_str}_to_{end_str}.xlsx"
    ...
    try:
        rows = _build_at_summary(df, start_dt, end_dt)
        #print(f"DEBUG: sample Site ID values = {[r.get('Site ID') for r in rows[:5]]}")  # TEMP DEBUG
        ...
        # ── optional status drill-down filter ────────────────────
        if status_filter and status_filter in AT_STATUS_MAP:
            rows = [r for r in rows if str(r.get(status_filter, "")).strip() == "Pending"]
 
         # ── optional site id filter (case-insensitive, partial match) ──
        if site_id_filter:
            site_id_lower = site_id_filter.lower()
            rows = [
                r for r in rows
                if site_id_lower in str(r.get("Site ID", "")).lower()
                or site_id_lower in str(r.get("SR_Site ID", "")).lower()
            ]
 
        if not rows:
            return Response(
                {"error": "No Pending rows found for the given filter."},
                status=status.HTTP_404_NOT_FOUND,
            )
