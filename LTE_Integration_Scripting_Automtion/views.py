...
<<<<<<< HEAD
        circle = request.POST.get("Circle") ########################## circle for different frequency and configreation as per circle:- ['AP','KK', 'DEL',...]
=======
...
<<<<<<< HEAD
        ...
        columns_to_needed_fdd = list(dict.fromkeys([
            "sectorCarrierId", "configuredMaxTxPower", "noOfTxAntennas", "noOfRxAntennas",
            "sectorEquipmentFunctionId", "eUtranCellFDDId", "cellId", "crsGain",
            "dlChannelBandwidth", "earfcndl", "earfcnul", "Latitude", "Longitude",
            "physicalLayerCellIdGroup", "physicalLayerSubCellId", "rachRootSequence",
            "tac", "ulChannelBandwidth"
        ]))

        columns_to_needed_tdd = list(dict.fromkeys([
            "sectorCarrierId", "configuredMaxTxPower", "noOfTxAntennas", "noOfRxAntennas",
            "sectorEquipmentFunctionId", "eUtranCellFDDId", "cellId", "crsGain",
            "dlChannelBandwidth", "earfcndl", "Latitude", "Longitude",
            "physicalLayerCellIdGroup", "physicalLayerSubCellId", "rachRootSequence",
            "tac", "ulChannelBandwidth"
        ]))
        ...
        for _, row in lte_df.iterrows():
            ...
            if any(f in row.get("eUtranCellFDDId", "") for f in ["_F1_", "_F3_", "_F8_"]):
...
<<<<<<< HEAD
                    if any(tech in "".join(cell_ids) for tech in ["_F1_", "_F2_", "_F3_"]):
=======
                    if any(
...
>>>>>>> vinay_duke_work
            ...
            for node in unique_nodes:
<<<<<<< HEAD
                with open(os.path.join(base_path_url, f'{node}_REMOTE_INTEGRATION_SCRIPTS', 'LTE_4G', gps_mme_path), "a") as file:
=======
                with open(
                    os.path.join(
                        base_path_url,
...
>>>>>>> vinay_duke_work
            ...
            for node in unique_nodes:
<<<<<<< HEAD
                with open(os.path.join(base_path_url, f'{node}_REMOTE_INTEGRATION_SCRIPTS', 'LTE_4G', gpl_lms_path), "a") as file:
        ...
        ############################################################################### 5G Cell Scripts ##########################################################################
            ...
            sctp_5g_endpoint_path = os.path.join(node_dir_5g, f'3_5G Sctp Endpoint Creation.txt')
            with open(sctp_5g_endpoint_path, 'a') as file:
                file.write(sctp_5g_endpoint_creation + "\n")

            Standalone_LTE_TERM_Enter_NR_IP_path = os.path.join(node_dir_5g, f'4_Standalone_LTE_TERM_Enter NR IP.mos')
            ...
            OR_LTE_Relation_LTE_ONLY_site_Enter_Gnb_ID_path = os.path.join(node_dir_5g, f'6_OR_LTE-Relation-LTE-ONLY-site_Enter Gnb ID.txt')
            ...
            for node in nr_cell_df['gNodeBName'].unique():
                ...
                nr_cell_df.rename(
                    columns={
                    'bSChannelBwDL/UL' : 'bSChannelBwDL-UL'
                    },inplace=True
                )
                ...
                with open(nr_cell_df_path, 'a') as file:
                    file.write(cell_creation_5g_script.format(gNBId = gnbid, GNBDUFUNCTION_SCRIPT_ELEMENT = gnbdu_fuction_element, GNBCUCPFUNCTION_SCRIPT_ELEMENT=gnbcucp_fuction_element))
                    file.close()
...
<<<<<<< HEAD
            ...
            for node in nr_cell_df['gNodeBName'].unique():
                ...
                nr_cell_df_path = os.path.join(node_dir_5g, f"01_NR_TN_RN_Cell_Def_{node_name}_{datetime.now().strftime("%Y-%m-%d %H-%M-%S")}.txt")
                os.makedirs(node_dir_5g, exist_ok=True)
                gnbid = nr_cell_df['gNBId'].unique()[0]
                print(nr_cell_df)
                gnbdu_fuction_element = ''
                ...
                with open(nr_cell_df_path, 'a') as file:
                    file.write(NR_TN_RN_Cell_Def.format(gNBId = gnbid, TN_GNPDUFUNCTION_ELEMENT = gnbdu_fuction_element, TN_GNBCUCPFUNCTION_ELEMENT=gnbcucp_fuction_element)+ '\n')
            
                TN_02_IPV6creationforanchor_text = TN_02_IPV6creationforanchor
                os.makedirs(node_dir_5g, exist_ok=True)

                TN_02_IPV6creationforanchor_path = os.path.join(node_dir_5g, f'02_IPV6creationforanchor.txt')
                with open(TN_02_IPV6creationforanchor_path, 'a') as file:
                    file.write(TN_02_IPV6creationforanchor_text + "\n")

                TN_03_ENDCanchornode_ROTN_path = os.path.join(node_dir_5g, f'03_ENDCanchornode_ROTN.txt')
                with open(TN_03_ENDCanchornode_ROTN_path, 'a') as file:
                    file.write(TN_03_ENDCanchornode_ROTN + "\n")

                TN_04_FreqRelation_path = os.path.join(node_dir_5g, f'04_FreqRelation.txt')
                with open(TN_04_FreqRelation_path, 'a') as file:
                    file.write(TN_04_FreqRelation + "\n")

                TN_05_5G_LMS_GPL_ROTN_path = os.path.join(node_dir_5g, f'05_5G_LMS_GPL ROTN.txt')
