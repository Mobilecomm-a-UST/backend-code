...
import zipfile
from .models import LTELogTable
from .serializers import LTELogTableSerializer
...
from LTE_Integration_Scripting_Automtion.circles.DEL.DEL_COMISSIONING_SCRIPT import (
...
from LTE_Integration_Scripting_Automtion.circles.UPW.UPW_INTEGRATION_SCRIPT import (
...
from LTE_Integration_Scripting_Automtion.circles.UPW.UPW_COMISSIONING_SCRIPT import (
...
from LTE_Integration_Scripting_Automtion.circles.HR.HR_INTEGRATION_SCRIPT import (
    HR_Route_4G_GPL_LMS,
    HR_TN_RN_GPS_MME,
    CISCO_MME_SCRIPT_HR,
    NOKIA_MME_SCRIPT_HR,
...
from LTE_Integration_Scripting_Automtion.circles.HR.HR_COMISSIONING_SCRIPT import (
...
from LTE_Integration_Scripting_Automtion.circles.CHN.CHN_INTEGRATION_SCRIPT import (
...
from LTE_Integration_Scripting_Automtion.circles.CHN.CHN_COMISSIONING_SCRIPT import (
    CHN_SITEBASIC_SCRIPT_BBU6630_BBU6631,
    CHN_SITEBASIC_SCRIPT_BBU6651,
    CHN_SITEEQUIPMENT_SCRIPT,
)

from LTE_Integration_Scripting_Automtion.circles.JK.JK_INTEGRATION_SCRIPT import (
...
from LTE_Integration_Scripting_Automtion.circles.JK.JK_COMISSIONING_SCRIPT import (
...
def send_email_integration_script(site_id_name,User_ID,circle,zip_filename):
    html_body = f"""
            <html>
            <head>
                <style>
            ...
            <body>
                <div class="container">
                    <div class="header">
                        LTE Script Generator Tool - Execution Summary
                    </div>

                    <p>Hello,</p>

                    <p>
                        The LTE Script Generator Tool has successfully generated the requested output files.
                    ...
                    <p>
                        The generated script package is available and ready for use.
                    </p>

                    <div class="footer">
                        This is an automated email generated by the LTE Script Generator Tool.
    ...
    """
    subject = f"LTE Script Generation Completed - {site_id_name}"
    to_email = ["282397@ust.com"]
    cc_email = []
    try:
        send_email(to_email, cc_email, subject, html_body, attachment_path=None, is_html=True)
...
def generate_integration_script(request):
    if request.method == "POST":
        ...
        #---------------------------------------------------------------- TN Circle-specific Script Generation ---------------------------------------------------------------------      
            ...
            for node in site_basic_df["eNodeBName"].unique():
                ...
                with open(site_equipment_script_path, "w") as file:
                    file.write(RBSSummary_script.format(
                        siteEquipmentFilePath=siteEquipmentFilePath,
                        siteBasicFilePath=siteBasicFilePath
                    ))
        #---------------------------------------------------------------- TN Circle-specific Script Generation ---------------------------------------------------------------------
        ...
        ############################################################################################################################################
        ###Log for current_user and mail###
        LTELogTable.objects.create(Site_ID = site_id_name,User_ID = User_ID,CIRCLE = circle,output_file_path = zip_filename)
        send_email_integration_script(site_id_name, User_ID,circle,zip_filename)
        ##=======================##
        return Response(
            {
                "status": True,
    ...
    if request.method == "GET":
        objs = LTELogTable.objects.all()
        serializer = LTELogTableSerializer(objs, many=True)
