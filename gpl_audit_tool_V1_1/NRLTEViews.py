...
def get_LTE_NR_GPL_AUDIT_commands(command_lines):
    ...
    # Define improved regex patterns — exact to file
    tasks = [
        (r"#+Start:\s*enbinfo\s*Audit\s*#+", r"#+\s*END\s+of\s+enbinfo\s*Audit\s*#+", "enbinfo"),
        (r"#+Start:\s*NR\s*gnbid\s*DATA\s*#+", r"#+\s*END\s+OF\s+NR\s+gnbid\s*DATA\s*#+", "gnbinfo"),
        ...
        # (r"#+Start:\s*LTE\s*GPL\s*Audit\s*#+", r"#+\s*END\s+of\s+LTE\s+GPL\s+Audit\s*#+", "LTE_GPL"),
        # (r"#+\s*NR\s*Baseline\s*Audit\s*#+", r"#+\s*END\s+of\s+NR\s+Baseline\s*Audit\s*#+", "NR_GPL"),
        # (r"#+Start:\s*Relation\s*Audit\s*#+", r"#+\s*END\s+of\s+Relation\s*Audit\s*#+", "Relation_Audit"),
        # (r"#+Start:\s*FeatureState\s*Audit\s*#+", r"#+\s*END\s+of\s+FeatureState\s*Audit\s*#+", "LTE_FeatureState"),
        (r"#+Start:\s*NR\s*FeatureState\s*Audit\s*#+", r"#+\s*END\s+of\s+NR\s*FeatureState\s*Audit\s*#+", "NR_FeatureState"),
        ...
        # (r"#+Start:\s*NR\s*Relation\s*Audit\s*#+", r"#+\s*END\s+of\s+NR\s+Relation\s*Audit\s*#+", "NR_Relation"),
    ]

    def extract_block(start_pat, end_pat):
    ...
    with ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(extract_block, start, end): key
            for start, end, key in tasks
        }
...
# =====================================================================================================
def generate_the_readlines(file_path):
    """Lazily read log file line by line."""
    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            yield line.strip()

# =====================================================================================================
def get_dataframe(command, file_path):
    """Extracts table from file based on command."""
    lines = list(generate_the_readlines(file_path))
    extractor = TableExtractor(lines)
    ...
    data = []
    for row in table[1:]:
        parts = [val.strip() for val in str(row).split(";")]
        parts = (parts + [""] * (max_cols - len(parts)))[:max_cols]
        data.append(parts)
...
def Lte_nR_extract_tables(extractor, result):
    """Process LTE/NR result command blocks into DataFrames."""
    dataframes = {}
    for key, commands in result.items():
        ...
        for command in commands:
            ...
            if table:
                headers = [col.strip() for col in table[0].split(";")]
                max_cols = len(headers)
                data = [[i.strip() for i in str(row).split(";")] for row in table[1:]]
                data = [
                ...
                if key == "LTE_GPL":
                    ...
                    df["Value"] = df["Value"].apply(lambda x: str(x).split(" ")[0])
                    # df.drop_duplicates(subset=["MO", "Parameter"], inplace=True)
        ...
        if temp_dfs:
            ...
            if key in ["NR_CELL_DATA", "LTE_CELL_DATA"]:
                merged_df = pd.concat(temp_dfs, axis=1, keys=["left", "right"])
                merged_df.columns = [
                    f"{x[0]}_{x[1]}" if isinstance(x, tuple) else x for x in merged_df.columns
                ]
                if 'right_Node_ID' in merged_df.columns:
...
# =====================================================================================================
def extract_node(file_path):  # noqa: F811
    """Extract node name from file name or contents."""
    base = os.path.basename(file_path)
    return os.path.splitext(base)[0]

def process_files(file, log_path, result):
    """Process single log file using TableExtractor."""
    lines = list(generate_the_readlines(log_path))
    extractor = TableExtractor(lines)
    dfs = Lte_nR_extract_tables(extractor, result)
    return file, dfs

# =====================================================================================================
@api_view(['POST'])
def get_LTE_NR_pre_post_audit(request):
    """
    API to perform 4G + 5G GPL pre/post audit.
    Compatible with WSGI servers like Waitress.
    """
    ...
    # ------------------- Load Command File -------------------
    command_file_path = os.path.join(base_dir, '4G+5G_command_file', 'GPL_Audit_command_NR.txt')
    if not os.path.exists(command_file_path):
        return Response({"error": f"Command file not found at {command_file_path}"}, status=404)

    with open(command_file_path, 'r') as f:
        command_lines = [line.strip() for line in f.readlines()]


    results = get_LTE_NR_GPL_AUDIT_commands(command_lines)
    
    print(results.keys())
    # json_file = os.path.join(base_dir, "commands_result.json")
    # with open(json_file, 'w', encoding='utf-8') as file:
    #     json.dump(results, file, indent=4, ensure_ascii=True)

    # ------------------- Handle Uploaded Files -------------------
    pre_log_files = request.FILES.getlist('pre_log_files')
    post_log_files = request.FILES.getlist('post_log_files')
    if not pre_log_files or not post_log_files:
        return Response({"error": "Both pre_log_files and post_log_files are required."}, status=400)

    current_time = datetime.now().strftime('%Y%m%d_%H%M%S')
    session_folder = os.path.join(base_dir, 'GPL_4G_5G_Audits', f"GPL_4G_5G_AUDIT_{current_time}")
    ...
    # ------------------- Extract Node Names -------------------
    pre_nodes = {extract_node(p): p for p in pre_paths}
    post_nodes = {extract_node(p): p for p in post_paths}
    ...
    post_cell_id_df = all_post_merged_df["LTE_CELL_DATA"][
    ...
    pre_cell_id_df = all_pre_merged_df["LTE_CELL_DATA"][
        ["left_Node_ID", "left_MO", "left_cellId"]
    ].rename(
        columns={
    ...
    ##################################################### for NRCELL ######################################################
    post_nr_cell_id_df = all_post_merged_df["NR_CELL_DATA"][
        ["left_Node_ID", "left_MO", "left_cellLocalId"]
    ].rename(columns={"left_Node_ID": "Post SiteId","left_MO": "Post CellName","left_cellLocalId": "cellLocalId",})
    pre_nr_cell_id_df = all_pre_merged_df["NR_CELL_DATA"][
        ["left_Node_ID", "left_MO", "left_cellLocalId"]
    ].rename(
        columns={
            "left_Node_ID": "Pre SiteId",
            "left_MO": "Pre CellName",
            "left_cellLocalId": "cellLocalId",
        }
    )
    ...
    pre_nr_df = all_pre_merged_df["gnbinfo"]
    pre_nr_df = pre_nr_df[pre_nr_df['MO'].str.startswith('GNBCUCPFunction=1')]
    pre_nr_gnodeid_mapping = {
        row["Node_ID"]: row["gNBId"]
        for _, row in pre_nr_df.iterrows()
    }
    ...
    post_nr_df = all_post_merged_df["gnbinfo"]
    post_nr_df = post_nr_df[post_nr_df['MO'].str.startswith('GNBCUCPFunction=1')]
    post_nr_gnodeid_mapping = {
        row["Node_ID"]: row["gNBId"]
        for _, row in post_nr_df.iterrows()
    }
    #####################################################################################################################################################
    
    #------------------------------------------------------------------ Merge pre and post data ---------------------------------------------------------#
    pre_cell_id_df["cellId"] = pre_cell_id_df["cellId"].astype(str)
    pre_nr_cell_id_df["cellLocalId"] = pre_nr_cell_id_df["cellLocalId"].astype(str)
    post_cell_id_df["cellId"] = post_cell_id_df["cellId"].astype(str)
    post_nr_cell_id_df["cellLocalId"] = post_nr_cell_id_df["cellLocalId"].astype(str)
    all_post_merged_df["Summary"] = pd.merge(pre_cell_id_df, post_cell_id_df, on=["cellId"], how="outer").fillna("NA")
    all_post_merged_df["NR_Summary"] = pd.merge(pre_nr_cell_id_df, post_nr_cell_id_df, on=["cellLocalId"], how="outer").fillna("NA")
    
    all_post_merged_df["Summary"] = all_post_merged_df["Summary"].sort_values(by=["Pre SiteId", "Pre CellName", "cellId"], ascending=True)
    all_post_merged_df["NR_Summary"] = all_post_merged_df["NR_Summary"].sort_values(by=["Pre SiteId", "Pre CellName", "cellLocalId"], ascending=True)
    
    ##################################################################################### Adding enode id ###############################################
    
    all_post_merged_df["Summary"]["Post eNBID"] = all_post_merged_df["Summary"]["Post SiteId"].apply(lambda x: post_enodeid_mapping[x] if x != "NA" else "NA")
    all_post_merged_df["NR_Summary"]["Post gNBId"] = all_post_merged_df["NR_Summary"]["Post SiteId"].apply(lambda x: post_nr_gnodeid_mapping[x] if x != "NA" else "NA")

    
    all_post_merged_df["Summary"]["Pre eNBID"] = all_post_merged_df["Summary"]["Pre SiteId"].apply(lambda x: pre_enodeid_mapping[x] if x != "NA" else "NA")
    all_post_merged_df["NR_Summary"]["Pre gNBId"] = all_post_merged_df["NR_Summary"]["Pre SiteId"].apply(lambda x: pre_nr_gnodeid_mapping[x] if x != "NA" else "NA")

    all_post_merged_df["Summary"].insert(5, "Post eNBID", all_post_merged_df["Summary"].pop("Post eNBID"))
    all_post_merged_df["NR_Summary"].insert(5, "Post eNBID", all_post_merged_df["NR_Summary"].pop("Post gNBId"))
    all_post_merged_df["Summary"].insert(2, "Pre eNBID", all_post_merged_df["Summary"].pop("Pre eNBID"))
    all_post_merged_df["NR_Summary"].insert(2, "Pre eNBID", all_post_merged_df["NR_Summary"].pop("Pre gNBId"))
    gpl_pre_post_file_path = os.path.join(session_folder, f"GPL_AUDIT_LTE_NR_Parameter_AReport_{current_time}.xlsx")
    ...
    with pd.ExcelWriter(gpl_pre_post_file_path, engine="xlsxwriter") as writer:
        ...
        for sheet_name, df in all_post_merged_df.items():
            if sheet_name == "Summary":
                ...
                nr_df = all_post_merged_df["NR_Summary"]
                nr_df.to_excel(
                    writer,
                    sheet_name=sheet_name,
                    startcol=9,
                ...
                format_excel_sheet(writer, sheet_name, nr_df, startrow=next_row, startcol=9)

            elif sheet_name in ["LTE_GPL", "NR_GPL"]:

                def create_new_cell_mapping(cell_data_df: pd.DataFrame):
                    return ({row['MO'] : row['cellId'] for _,row in cell_data_df.iterrows()})
                
                def create_new_nr_cell_mapping(cell_data_df: pd.DataFrame):
                    return ({row['MO'] : row['cellLocalId'] for _,row in cell_data_df.iterrows()})
                ...
                def create_nr_cell_mappings(summary_df):
                    return (
                        {
                            row["Pre CellName"]: row["cellLocalId"]
                            for _, row in summary_df.iterrows()
                        },
                        {
                            row["Post CellName"]: row["cellLocalId"]
                            for _, row in summary_df.iterrows()
                        },
                        {
                            row["Pre SiteId"]: row["Post SiteId"]
                ...
                def add_nr_cell_ids(df: pd.DataFrame, col_name, mapping):
                    df.insert(1, "cellLocalId", df["MO"].apply(lambda x: mapping.get(x.split(",")[0], 0)))
                    return df

                if(sheet_name == "LTE_GPL"):
                    cell_id_df = all_post_merged_df["Summary"].sort_values(by="Pre SiteId")

                    cell_data_df = all_post_merged_df["LTE_CELL_DATA"]
                    ...
                    merged_df["Parameter Setting Status"] = merged_df.apply(lambda row: ("OK" if row["Current value"] == row["Pre-existing Value"] else ("Missing" if pd.isna(row["Current value"]) or pd.isna(row["Pre-existing Value"]) else "NOT OK")),axis=1)
                    ...
                    merged_df = merged_df[["Node_ID", "MO"] + [col for col in merged_df.columns if col not in ["Node_ID", "MO"]]]
                    merged_df.to_excel(writer, sheet_name=sheet_name, index=False)
                    format_excel_sheet(writer, sheet_name, merged_df, startrow=0, startcol=0)

                elif sheet_name == 'NR_GPL':
                    nr_cell_id_df = all_post_merged_df["NR_Summary"].sort_values(by="Pre SiteId")
                    nr_cell_data_df = all_post_merged_df["NR_CELL_DATA"]
                    # pre_node_name = next((re.sub(r'^x+', '', str(x).split('=')[-1].split('_')[-2]) for x in nr_cell_id_df['Pre CellName'] if isinstance(x, str) and '=' in x and len(x.split('=')[-1].split('_')) > 1), None)
                    pre_node_name = next((str(x).split('=')[-1].split('_')[-2] for x in nr_cell_id_df['Pre CellName'] if isinstance(x, str) and '=' in x and len(x.split('=')[-1].split('_')) > 1), None)
                    # post_node_name = next((re.sub(r'^x+', '', str(x).split('=')[-1].split('_')[-2]) for x in nr_cell_id_df['Post CellName'] if isinstance(x, str) and '=' in x and len(x.split('=')[-1].split('_')) > 1), None)
                    post_node_name = next((str(x).split('=')[-1].split('_')[-2] for x in nr_cell_id_df['Post CellName'] if isinstance(x, str) and '=' in x and len(x.split('=')[-1].split('_')) > 1), None)
                    gpl_pre_df = all_pre_merged_df.get(sheet_name)
                    gpl_post_df: pd.DataFrame = df.copy()
                    gpl_pre_df = gpl_pre_df.assign(
                        **{
                    ...
                    pre_map, post_map, site_map = create_nr_cell_mappings(nr_cell_id_df)
                    post_map_cellId = create_new_nr_cell_mapping(nr_cell_data_df)
                    gpl_pre_df['MO'] = gpl_pre_df['MO'].apply(lambda x: str(x).replace(f"_{pre_node_name}", f"_{post_node_name}"))
                    gpl_pre_df = add_nr_cell_ids(gpl_pre_df, "MO", post_map_cellId)
                    gpl_post_df = add_nr_cell_ids(gpl_post_df, "MO", post_map)
                    merged_df = pd.merge(left=gpl_pre_df, right=gpl_post_df, how='left', on=['MO','cellLocalId', 'Parameter'], indicator=True)
                    merged_df["Node_ID_y"] = merged_df["Node_ID_y"].fillna("Cell is not Found in Post")
                    merged_df["Current value"] = merged_df['Value_y']
                    merged_df.drop(columns=["Node_ID_x", "Value_x", "Value_y"], inplace=True)
                    merged_df.rename(columns={"Node_ID_y": "Node_ID"}, inplace=True)
            ...
            elif sheet_name in ["LTE_FeatureState", "NR_FeatureState"]:
                feature_pre_df: pd.DataFrame = all_pre_merged_df.get(sheet_name)
                print("deature state in case of NR feature:- \n",feature_pre_df)
                feature_pre_df.rename(columns={'featureState' : 'Pre Existing FeatureState', 'licenseState': 'Pre Existing LicenseState'}, inplace=True)
                cell_id_df = all_post_merged_df["Summary"]
                feature_pre_df["Current FeatureState"] = ""
                feature_pre_df["Pre Existing FeatureState"] = feature_pre_df["Pre Existing FeatureState"].astype(str)
                feature_pre_df["Pre Existing LicenseState"] = feature_pre_df["Pre Existing LicenseState"].astype(str)
                feature_pre_df["Pre Existing FeatureState"] = feature_pre_df["Pre Existing FeatureState"].apply(lambda x: x.split(" ")[0] if " " in x else x)
                feature_pre_df["Pre Existing LicenseState"] = feature_pre_df["Pre Existing LicenseState"].apply(lambda x: x.split(" ")[0] if " " in x else x)
                ...
                print("feature in case of the post logs:- \n",feature_post_df)
                ...
                for node_id in feature_pre_df["Node_ID"].unique():
                    ...
                    post_df = feature_post_df[feature_post_df["Node_ID"] == post_node_id]
                    commind_df = pd.merge(left=pre_df, right=post_df, on=["CXC ID", "description"], how="left")
                    commind_df["Node_ID_y"] = commind_df["Node_ID_y"].fillna(commind_df["Node_ID_x"])
                    ...
                    commind_df.rename(columns={"Node_ID_x": "Node_ID"}, inplace=True)
                    commind_df.drop(columns=["Node_ID_y", "featureState", "licenseState"],inplace=True)
                    commind_df["Current FeatureState"] = commind_df["Current FeatureState"].astype(str)
                    commind_df["Current LicenseState"] = commind_df["Current LicenseState"].astype(str)
                    merged_df = pd.concat([merged_df, commind_df], axis=0, ignore_index=True)
            ...
            # elif sheet_name == "NR_Relation":
            #     pre_nr_relation_df = all_pre_merged_df.get(sheet_name)
            #     post_nr_relaton_df = all_post_merged_df.get(sheet_name)
                
                
                
            #     print(pre_nr_relation_df['MO'])
            #     print(post_nr_relaton_df['MO'])
                
            #     exit(0)
                
            else:
                if sheet_name == "LTE_CELL_DATA":
                    df.rename(
                        columns={
                ...
                elif sheet_name == "NR_CELL_DATA":
                    df.rename(columns={'right_MO': 'right_SectorCarrierID','right_sectorEquipmentFunctionRef': 'right_SectorId'},inplace=True)

                    df.columns = [col.replace("left_", "") if col.startswith("left_") else col.replace("right_", "") for col in df.columns]

                    required_columns = [
                        'Node_ID', 'MO', 'administrativeState', 'bandListManual',
                        'cellLocalId', 'nRPCI', 'nRTAC', 'operationalState',
                        'rachRootSequence', 'ssbFrequency', 'userLabel',
                        'SectorId', 'SectorCarrierID', 'configuredMaxTxPower',
                        'noOfUsedRxAntennas', 'noOfUsedTxAntennas', 'reservedBy'
                    ]
                    df = df[[col for col in required_columns if col in df.columns]]
