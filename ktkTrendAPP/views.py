...
def G2_trend(df_2G_raw_kpi,df_site_list_2G,trend_wb,offered_date,df_mapping_file):
    ...
    # printString(27906)
        ...
        exclude_cols = ["Site Name_x", "Cell Name", "Integration", "Date", "Site Name", "Site ID"]
        ...
        df_pivoted_2G = G2_filter_df.pivot_table(
            index=["Site Name_x", "Cell Name", "Integration"],
            columns="Date",
            values=numeric_cols,
            aggfunc="mean"
        )
...
#####################################################################################
    ...
    # Filter relevant rows
            df_filtered = only_site_fil[
                ...
                (only_site_fil.CellName.str.contains('|'.join(tech)))
            ]
            ...
            if not df_filtered.empty:
                ...
                # Convert KPI columns to numeric
                exclude_cols = [
                    "SITE_ID", "CellName", "CELL_ID", "eNodeB Name",
                    "tech", "LocalCell Id", "Site Name", "Integration", "Date"
                ]
                ...
                df_pivoted = df_filtered.pivot_table(
                    index=[
                        "SITE_ID", "CellName", "CELL_ID", "eNodeB Name",
                        "tech", "LocalCell Id", "Site Name", "Integration"
                    ],
                    columns="Date",
                    values=kpi_cols,
                    aggfunc="mean"
...
#_____________________________________work for final file__________________________________
    ...
    ### KTK Trend New (for raw file in mycomm)------------------------------
       
   #fill zeros and round off function
...
def G2_trend_HUI(df_2G_raw_kpi_HUI,df_site_list_2G,trend_wb,offered_date,df_mapping_file):
        ...
        if 'Unnamed: 1' in df_2G_raw_kpi_HUI.columns:
          ...
          df_2G_raw_kpi_HUI["2G Site Name"] = df_2G_raw_kpi_HUI["2G Site Name"].str.replace(r"^2G_", "", regex=True).str.replace(r".*_2G_", "", regex=True)
          df_2G_raw_kpi_HUI["2G Cell Name"] = ( df_2G_raw_kpi_HUI["2G Cell Name"] .str.replace(r"^2G_", "", regex=True).str.replace(r".*_2G_", "", regex=True).str.replace(r"^[EZ]", "", regex=True)  
)
        ...
        def overwrite(df_pivoted,kpi_name,coln1,trend_ws):
            ...
            for i,value in enumerate(index_pivot):
                j=i+4  
                trend_ws["H"+str(j)].value=date1.strftime("%Y-%m-%d")
                trend_ws["I"+str(j)].value="Done"
                trend_ws["E"+str(j)].value="G900"
                trend_ws["F"+str(j)].value="Done"
                trend_ws["D"+str(j)].value="2G+FDD"
    ...
    # Filter relevant rows
            ...
            if not df_filtered.empty:
                ...
                exclude_cols = [
                    ...
                    "tech", "Ericsson_4G.ECELLID", "Site Name", "Integration", "Date"
                ]
                kpi_cols = [col for col in df_filtered.columns if col not in exclude_cols]
                df_filtered[kpi_cols] = df_filtered[kpi_cols].apply(pd.to_numeric, errors="coerce")
                ...
                df_pivoted = df_filtered.pivot_table(
                    index=[
                        "SITE_ID", "Shortname", "CELL_ID",
                        "tech", "Site Name", "Integration","Ericsson_4G.ECELLID"
                    ],
                    columns="Date",
                    values=kpi_cols,
                    aggfunc="mean"
        ...
        def overwrite(df_pivoted,kpi_name,coln1,trend_ws):
            ...
            for i, value in enumerate(index_pivot):
                ...
                trend_ws["C" + str(j)].value = index_pivot[i][6] if len(index_pivot[i]) > 6 else ''
