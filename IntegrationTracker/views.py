...
def upload_excel(request):
    ...
    if value_in_cell == "mcom_v1.7":
            ...
            allowed_values = {
                ...
                'Activity Name': ['ULS_HPSC', 'RELOCATION', 'MACRO', 'DE-GROW', 'RET', 'IBS', 'ODSC', 'IDSC', 'HT INCREMENT', 'FEMTO', 'OTHERS', 'UPGRADE','RECTIFICATION','5G SECTOR ADDITION','OPERATIONS','5G RELOCATION','TRAFFIC SHIFTING','RRU_UPGRADE'],
        ...
        query = f"""
            select * from (
                select * from crosstab($$ 
                    ...
                    CROSS JOIN
                        (SELECT unnest(ARRAY[
                            '5G SECTOR ADDITION','5G RELOCATION', '5G BW UPGRADE', '5G RRU SWAP', 'DE-GROW', 'FEMTO', 'HT INCREMENT', 
                            'IBS', 'IDSC', 'MACRO', 'ODSC','OPERATIONS','OTHERS','RECTIFICATION', 
                            'RELOCATION', 'RET','TRAFFIC SHIFTING', 'ULS_HPSC', 'UPGRADE' ,'RRU_UPGRADE'
                        ]) AS "Activity_Name") AS "Activity_Name"
                    LEFT JOIN (
                    ...
                    order by 1,2 
                $$) as ct(
                    cir text,"D1_5G_RELOCATION" INTEGER,"D1_5G_SECTOR_ADDITION" INTEGER ,"D1_DE_GROW" INTEGER,
                    "D1_FEMTO" INTEGER,"D1_HT_INCREMENT" INTEGER,"D1_IBS" INTEGER,"D1_IDSC" INTEGER,
                    "D1_MACRO" INTEGER,"D1_ODSC" INTEGER,"D1_OPERATIONS" INTEGER,"D1_OTHERS" INTEGER,
                    "D1_RECTIFICATION" INTEGER,"D1_RELOCATION" INTEGER,"D1_RET" INTEGER,"D1_TRAFFIC_SHIFTING" INTEGER,
                    "D1_ULS_HPSC" INTEGER,"D1_UPGRADE" INTEGER,"RRU_UPGRADE" INTEGER, "D1_5G_BW_UPGRADE" INTEGER, "D1_5G_RRU_SWAP" INTEGER
                )
            ) as d1
            ...
            FULL OUTER JOIN (
                select * from crosstab($$ 
                    ...
                    order by 1,2 
                $$) as ct(
                    cir text,"D2_5G_RELOCATION" INTEGER,"D2_5G_SECTOR_ADDITION" INTEGER,"D2_DE_GROW" INTEGER,
                    "D2_FEMTO" INTEGER,"D2_HT_INCREMENT" INTEGER,"D2_IBS" INTEGER,"D2_IDSC" INTEGER,
                    "D2_MACRO" INTEGER,"D2_ODSC" INTEGER,"D2_OPERATIONS" INTEGER,"D2_OTHERS" INTEGER,
                    "D2_RECTIFICATION" INTEGER,"D2_RELOCATION" INTEGER,"D2_RET" INTEGER,"D2_TRAFFIC_SHIFTING" INTEGER,
                    "D2_ULS_HPSC" INTEGER,"D2_UPGRADE" INTEGER,"RRU_UPGRADE" INTEGER, "D1_5G_BW_UPGRADE" INTEGER, "D1_5G_RRU_SWAP" INTEGER
                )
            ) as d2
            using("cir")
            ...
            FULL OUTER JOIN (
                select * from crosstab($$ 
                    ...
                    order by 1,2 
                $$) as ct(
                    cir text,"D3_5G_RELOCATION" INTEGER,"D3_5G_SECTOR_ADDITION" INTEGER,"D3_DE_GROW" INTEGER,
                    "D3_FEMTO" INTEGER,"D3_HT_INCREMENT" INTEGER,"D3_IBS" INTEGER,"D3_IDSC" INTEGER,
                    "D3_MACRO" INTEGER,"D3_ODSC" INTEGER,"D3_OPERATIONS" INTEGER,"D3_OTHERS" INTEGER,
                    "D3_RECTIFICATION" INTEGER,"D3_RELOCATION" INTEGER,"D3_RET" INTEGER,"D3_TRAFFIC_SHIFTING" INTEGER,
                    "D3_ULS_HPSC" INTEGER,"D3_UPGRADE" INTEGER,"RRU_UPGRADE" INTEGER, "D1_5G_BW_UPGRADE" INTEGER, "D1_5G_RRU_SWAP" INTEGER
                )
            ) as d3
            using("cir")
        """
...
def date_range_wise_integration_data(request):
        ...
        with connection.cursor() as cursor:
            query = f"""
                SELECT * FROM crosstab($$
                    ...
                    CROSS JOIN
                        (SELECT unnest(ARRAY[
                            '5G SECTOR ADDITION','5G RELOCATION','DE-GROW', 'FEMTO', 'HT INCREMENT', 'IBS',
                            'IDSC', 'MACRO', 'ODSC', 'OPERATIONS','OTHERS','RECTIFICATION', 'RELOCATION',
                            'RET', 'TRAFFIC SHIFTING', 'ULS_HPSC', 'UPGRADE', 'RRU_UPGRADE'
                        ]) AS "Activity_Name") AS "Activity_Name"
                    LEFT JOIN
                        (
                            SELECT "CIRCLE", "Activity_Name", COUNT("id") AS cnt 
    ...
    async def get_data_async():
        results, columns = await sync_to_async(fetch_crosstab_data)()
...
def monthwise_integration_data(request):
    ...
    with connection.cursor() as cursor:
        ...
        query = f""" 
            ...
            group by "CIRCLE","Activity_Name") as r
                USING ("CIRCLE", "Activity_Name") order by 1,2 $$) as 
            ct(cir text,  "M3_5G_RELOCATION" INTEGER, "M3_5G_SECTOR_ADDITION" INTEGER,"M3_DE_GROW" INTEGER,"M3_FEMTO" INTEGER,"M3_HT_INCREMENT" INTEGER,"M3_IBS" INTEGER,"M3_IDSC" INTEGER,"M3_MACRO" INTEGER,"M3_ODSC" INTEGER,"M3_OPERATIONS" INTEGER ,"M3_OTHERS" INTEGER, "M3_RECTIFICATION" INTEGER, "M3_RELOCATION" INTEGER,"M3_RET" INTEGER, "M3_TRAFFIC_SHIFTING" INTEGER,"M3_ULS_HPSC" INTEGER,"M3_UPGRADE" INTEGER,"RRU_UPGRADE" INTEGER)) as m3
            using("cir")
...
def integration_table_update(request, id=None):
    user = request.user.username
    print("username: ", user)
    nokia_spocks=['chandan.kumar@mcpsinc.com','girraj.singh@mcpsinc.in','mohit.batra@mcpsinc.com','abhishek.gupta']
