...
save_directory = os.path.join(MEDIA_ROOT, "SAMSUNG_OUTPUT", "attachments")
LAST_PROCESSED_FILE = os.path.join(MEDIA_ROOT, "SAMSUNG_OUTPUT", "last_mail.txt")
...
def get_summary_counts(excel_path):
    ...
    if old_new_4g_df is not None:
        ...
        for _, row in old_new_4g_df.iterrows():
            ...
            old_cell = str(
                row.get("user-label_old", "")
            ).strip()

            new_cell = str(
                row.get("user-label_new", "")
            ).strip()
    ...
    if alarm_df is not None:
        ...
        for _, row in alarm_df.iterrows():
            ...
            if status == "new site":
                ...
                site = row.get("SiteId", "")
                LNBTS_ID = row.get("LNBTS ID", "")

                if (
                    pd.notna(site)
                    and pd.notna(LNBTS_ID)
                ):

                    site = str(site).strip().upper()
                    LNBTS_ID = str(LNBTS_ID).strip().upper()
    ...
    if alarm_df is not None:
        ...
        for _, row in alarm_df.iterrows():
            ...
            if status == "new site":
                ...
                site = row.get("SiteId", "")
                NRBTS_ID = row.get("NRBTS ID", "")

                if (
                    pd.notna(site)
                    and pd.notna(NRBTS_ID)
                ):

                    site = str(site).strip().upper()
                    NRBTS_ID = str(NRBTS_ID).strip().upper()
    ...
    if (
        ...
        and "LNBTS ID" in alarm_df.columns
    ...
    if (
        "No Alarms/Service Affecting Alarms/Non-Service Affecting Alarms" in alarm_df.columns
        and "NRBTS ID" in alarm_df.columns
    ...
    if (
        "5G No Alarms/Service Affecting Alarms/Non-Service Affecting Alarms" in alarm_df.columns
        and "NRBTS ID" in alarm_df.columns
        and "5G Alarm Status" in alarm_df.columns
    ):
        ...
        site_down_5g_new = alarm_df[
            (
                alarm_df["5G No Alarms/Service Affecting Alarms/Non-Service Affecting Alarms"]
                .astype(str)
                .str.contains(
                    "SITE DOWN",
                    case=False,
...
@shared_task
def outlook_mail(expected_subject="Samsung Alarm"):
...
def process_daily_circle_logs(file_path, to_address=None, cc_mails=None):
    print("Processing Samsung Alarm ZIP...")
    ...
    response = requests.post(
        "http://127.0.0.1:8001/oem_samsung/Samsung/",
        files=files_payload
    )
    ...
    if response.status_code == 200:
        ...
        reply_subject = "📊 Samsung Alarm Status"
        circle_mail_map = {
            # "PUN":["Abhinav.Verma@ust.com"]
            "PUN": ["Ashafaq.Husain@ust.com","Nirvay.Mishra@ust.com", "Anil.Sharma@ust.com"],
            "KOL": ["Nityananda.Halder@ust.com", "Tanuj.Singh@ust.com", "Masud.Rana@ust.com","Sushovan.Pal@ust.com"],
        }
