...
def LKF_Upload(request):
    ...
    for uploaded_file in saved_files:
        ...
        sectorcarrier_df = explode_data_from_log(
                r'[A-Z0-9_-]+>\sget\s\.\ssectorc(?:arrier)?',
                r'MO\s+Attribute\s+Value',
                r'^\s*(\S+)\s+(\S+)\s+(.*)$',
                r'^Total:\s\d+',
                processed_content
    ...
    lambda x: re.search(r'(SectorCarrier=\d+)', x).group(1) if re.search(r'(SectorCarrier=\d+)', x) else None)
        ...
        fingerprint_df = explode_data_from_log(
            r'[A-Z0-9_-]+>\sget\s\.\sfing(?:erprint)?',
            r'MO\s+Attribute\s+Value',
            r'^\s*(\S+)\s+(\S+)\s+(.*)$',
            r'^Total:\s\d+',
            file_content
                ...
                if sheet_name == "get maxtx" and not xls.parse(sheet_name).empty:
                    ...
                    if not df_maxtx.empty and not df_sectorc.empty:
                        ...
                        for mo in common_mo:
                            ...
                            if len(power_vals) > 0 and pd.notna(power_vals[0]):
                                power_tx_map[mo] = int(power_vals[0])
                        ...
                        for _, row in df_sectorc.iterrows():
                            mo = row["newValue"]
                            band_info = row["MO"]
                            MIMO_map = { "F1": "L2100", "F3": "L1800", "F8": "L900", "T1": "L23001", "T2": "L23002", "F5": "L85", }
                            ...
                            if match and mo in power_tx_map:
                                ...
                                if pair not in seen_pairs:
                                    ...
                                    power_watt = power_tx_map[mo] // 1000
                                    key = f"{power_watt}W_{mimo_layer}"
                                    layer_counter[key] += 1


                    # Process layer_counter correctly----------
                    for key, count in layer_counter.items():
                        match = re.match(r"(\d+)W_(L\d+)", key)
                        if not match:
                            continue
                    ...
                    for sheet_name in xls.sheet_names:
                        ...
                        if sheet_name == "st trx" and not xls.parse(sheet_name).empty:
                            ...
                            df["Cell_Name"] = df["MO"].str.split("EUtranCellTDD=").str[-1]
                            df["tech"] = df["Cell_Name"].str.split("_").str.get(2)
                            tech_counts = df["tech"].value_counts()
                            for tech, col_name in tech_map.items():
                                template_df.loc[0, col_name] = tech_counts.get(tech, 0)
                                template_df.replace(0, pd.NA, inplace=True)
                            ...
                            df['site_id'] = df['MO'].str.extract(r'_([A-Z0-9]+?)[A-Z]?(?=_[A-Z]$)')
