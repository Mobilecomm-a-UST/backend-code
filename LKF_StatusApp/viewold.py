...
#find the template file----
       ...
       #find the baseband and rru              
        ...
        #             ## Process the 'Layer' column based on the 'MO' column
        ...
        #                     layers.append(value)
        #             # TDD condition (if either T1 or T2 present)
        #             if "T1" in layer_matches or "T2" in layer_matches:
        #                 layers.append("TDD")
        #             # 5G layer check
        #             has_5g = df["MO"].str.contains("NRCellDU").any()
        #             if has_5g:
        #                 layers.append("5G")
        ...
        #             template_df.loc[0, "TRX"] = trx
                ...
                # if trx and trx > 1:
                #     existing_layer = template_df.loc[0, "Layer"]
                #     updated_layer = (existing_layer + "+TRX") if existing_layer else "TRX"
                #     template_df.loc[0, "Layer"] = updated_layer
        ...
        if "MO Name" in merged_df.columns and "Node IP" in merged_df.columns:
            merged_df = merged_df[~(
                (merged_df["MO Name"].isnull() | (merged_df["MO Name"] == "")) &
                (merged_df["Node IP"].isnull() | (merged_df["Node IP"] == "")) )]
