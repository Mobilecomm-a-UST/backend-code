...
def unifrom_shortname(df):
    mapping_sectornum_character = {
    '11': '_T2_A_A', '12': '_T2_B_B', '13': '_T2_C_C', '14': '_T2_D_D', '15': '_T2_A_L', '16': '_T2_B_M', '17': '_T2_C_N',
    '21': '_F3_A_A', '22': '_F3_B_B', '23': '_F3_C_C', '24': '_F3_D_D', '25': '_F3_A_L', '26': '_F3_B_M', '27': '_F3_C_N',
    '41': '_T1_A_A', '42': '_T1_B_B', '43': '_T1_C_C', '44': '_T1_D_D', '45': '_T1_A_L'
    }
    def append_character(shortname):
        # Get the last 2 characters from the short name
        last_two_characters = shortname[-2:]
        # Check if the last 2 characters exist in the mapping dictionary
        if "Sams" in shortname and last_two_characters in mapping_sectornum_character:
            # Append the corresponding value from the dictionary
            return shortname + mapping_sectornum_character[last_two_characters]
        else:
            return shortname  # Return the original short name if the last 2 characters are not found in the dictionary

    # Apply the function to the 'shortname' column
    df['Short name'] = df['Short name'].apply(append_character)



    def remove_tdd_fdd(string):
        if not "Sams" in string:
                    if string.endswith("TDD"):
                        return string[:-4]  # Remove "TDD" from the end of the string
                    elif string.endswith("FDD"):
                        return string[:-4]  # Remove "FDD" from the end of the string
                    else:
                        return string
        else:
            return string
            
    df['Short name']= df["Short name"].apply(remove_tdd_fdd)
    return df
...
def Degrow_trend(request):
    ...
    site_list_pre=[]
    for srtn in df_pre_file["Short name"]:
        if "Sams" in srtn :
#         print(srtn)
          site_list_pre.append(srtn.split(",")[1].split("_")[-1].strip())
#     Sams-PJ1,PB_E_F3XXXXXX_PHG707,cNum21
        else:
          site_list_pre.append(srtn.split("_")[-3][:-1].strip())
#         print(srtn,srtn.split("_")[-3][:-1])

    site_list_post=[]
    for srtn in df_post_file["Short name"]:
        if "Sams" in srtn :
#         print(srtn)
          site_list_post.append(srtn.split(",")[1].split("_")[-1].strip())
#     Sams-PJ1,PB_E_F3XXXXXX_PHG707,cNum21
        else:
          site_list_post.append(srtn.split("_")[-3][:-1].strip())
#         print(srtn,srtn.split("_")[-3][:-1])



    ################################## for uniform shortname as we have samsumng here in pb  ######################################
    df_pre_file=unifrom_shortname(df_pre_file)
    df_post_file=unifrom_shortname(df_post_file)
    ###################************************************#############################


    df_pre_file["Site_ID"]=site_list_pre
    df_post_file["Site_ID"]=site_list_post
    df_pre_file
    bulk_input=[]
