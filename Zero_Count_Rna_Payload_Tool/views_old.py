...
def ticket_counter_upload_report():
    ...
    for _, row in merged_df.iterrows():
        ...
        if Ticket_Counter_Table_Data.objects.filter(Short_name=short_name).exists():
            instance = list(
                Ticket_Counter_Table_Data.objects.filter(Short_name=short_name).values()
            )
            last_instance = instance[0]
...
def circle_wise_open_close_dashboard(request):
    objs = Ticket_Counter_Table_Data.objects.all()
    ...
    if from_date and to_date:
        ...
        objs = objs.filter(Date__range=[from_date, to_date])
    ...
    MAX_DATE = objs.aggregate(Max("Date"))["Date__max"]
    MIN_DATE = objs.aggregate(Min("Date"))["Date__min"]
